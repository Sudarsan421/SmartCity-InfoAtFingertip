...
<div class="main">
 ...
 <%try{
  ...
  String bookid="";
   int cnt=0;
  ResultSet rs=st.executeQuery("select * from booking where vid='"+vid+"' and  lat='"+lat+"' and long='"+long+"' and bookfromdate='"+bookfromdate+"' and  bookfromtime='"+bookfromtime+"' and uid='"+uid+"'");
  if(rs.next()){
  out.print("<font color=red size=3> Ambulance is Already Booked Successfully </font><br />");
	
  }else{
  rs=st.executeQuery("select max(substr(bookid,2)) from booking");
	try{
	   if(rs.next())
	      {
	     cnt=rs.getInt(1);
