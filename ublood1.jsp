...
<h4 class="title">Welcome <%= ((String)session.getAttribute("name")).toUpperCase() %> to Donate Blood!</h4>  <div class="article">
<br />
<%
try{
String oid=request.getParameter("bld");
String grp=request.getParameter("grp");
String rh=request.getParameter("rh");
String furdt=request.getParameter("furdt");
String id=(String)session.getAttribute("id");
String dt=new  java.text.SimpleDateFormat("yyyy-MM-dd").format(new java.util.Date());


String qry="select * from blddonation where uid='"+id+"'";
...
//out.print(data);
qry="select * from blddonation where uid='"+id+"' and (select dntdate from blddonation where uid='"+id+"' order by dntdate desc limit 1) < DATE_ADD( NOW( ) , INTERVAL -3 MONTH ) order by dntdate desc";
//out.print(qry);
...
if(rs.next() || data==0){
	...
	if(cnt>0 && cnt<10)
		...
		did="D"+cnt;
	qry="insert into blddonation values('"+did+"', '"+oid+"', '"+id+"', '"+grp+"', '"+rh+"','"+furdt+"','Pending')";
	//out.print(qry);
	st.executeUpdate(qry);
	out.print("<font color=green size=3>Blood Requisition Complete</font>");
}else{
	out.print("<font color=red size=+2>You have already given blood within last 3 month.</font>");
}
con.close();
}catch(Exception e){  }
%>	 
