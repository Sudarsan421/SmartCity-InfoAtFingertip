...
<br />
<form action="uplacephoto.jsp"> 
<span class="style2">Search by Place Name</span> 
<select name="a1">			   
<%
try{
...
<form action="uplacephoto.jsp">  
<span class="style2">Search by Famous Place</span>   
<select name="a1">			   
<%
try{
ResultSet rs = st.executeQuery("select * from place order by famfor asc");
while(rs.next()){
%>
	<option value="<%=rs.getString(1)%>"><%=rs.getString(6)+" : " +rs.getString(2)%></option>
<%
} 
}catch(Exception e){ }
%>
...
<form>  
<span class="style2">Search by District</span>   
<select name="a1">			   
<%
try{
ResultSet rs = st.executeQuery("select distinct dist from place");
while(rs.next()){
%>
	<option value="<%=rs.getString(1)%>"><%=rs.getString(1)+" : " +rs.getString(1)%></option>
<%
} 
}catch(Exception e){ }
%>
...
try{
ResultSet rs = st.executeQuery("select * from place where dist='"+request.getParameter("a1")+"'");
if(rs.next()){
rs.beforeFirst();
while(rs.next()){
%>
<a href="uplacephoto.jsp?a1=<%= rs.getString(1)%>"> <font color="#0000FF" size="2"><%= rs.getString(2).toUpperCase()%></font> </a> <br />
<%
}
}
con.close();
